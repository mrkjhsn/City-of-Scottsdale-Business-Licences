...
for row in inputfile:
    ...
    if index == 1:
        ...
        bus_name = row[0:(re.search(r'[,]\d{2,7}[,]', row).start())]  #pulls the business name off the front of the string, using the business number(between 2 and 7 digits with comma on either side) as the point of reference for the regex
